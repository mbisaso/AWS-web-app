...
type FieldErrors = Partial<
  Record<
    "email" | "first_name" | "last_name" | "password" | "non_field_errors",
    string[]
  >
>;
...
export function RegisterPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const res = (
        err as { response?: { data?: { error?: FieldErrors | string } } }
      ).response;
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-5rem)] max-w-7xl items-center justify-center">
        <div className="w-full max-w-2xl rounded-[32px] border border-slate-200/80 bg-white/80 p-6 shadow-[0_20px_70px_rgba(15,23,42,0.08)] backdrop-blur-sm sm:p-8 lg:p-10">
          <div className="mb-8 flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-sky-primary to-sky-deep text-lg font-black text-white shadow-md shadow-sky-200/40">
              A
            </div>
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.22em] text-sky-primary">
                WIMEA-ICT AWS
              </p>
              <h1 className="text-3xl font-semibold text-midnight font-display">
                Create an account
              </h1>
            </div>
          </div>
          ...
          <form className="grid gap-5" onSubmit={handleSubmit} noValidate>
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <div>
                <label
                  className="mb-2 block text-sm font-medium text-midnight"
                  htmlFor="first_name"
                >
                  First name
                </label>
                ...
                />
                {fieldErrors.first_name?.map((e) => (
                  <p key={e} className="mt-1 text-xs text-rose-600">
                    {e}
                  </p>
                ))}
              </div>
          ...
          <div className="mt-8 border-t border-slate-200 pt-6 text-sm text-storm/60">
            <p>
              ...
              <Link
                className="font-semibold text-sky-primary transition-colors hover:text-sky-deep"
                to="/login"
              >
                Log in
              </Link>
            ...
            <p className="mt-3">
              <Link
                className="font-medium text-storm/40 transition-colors hover:text-midnight"
                to="/"
              >
                Back to home
              </Link>
