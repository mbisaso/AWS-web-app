          ...
          <div className="rounded-[28px] border border-slate-200 bg-white shadow-[0_20px_70px_rgba(15,23,42,0.06)]">
            <div className="flex flex-col gap-4 border-b border-slate-200 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <h1 className="mt-2 text-2xl font-semibold text-[#1a2332]">
                  Station overview
                </h1>
              </div>
                  ...
                  <section>
                    ...
                    <div className="grid gap-4 md:grid-cols-3">
                      {(
                        ...
                        <article
                          key={status}
                          className="rounded-[28px] border border-slate-200 bg-[#f8fafc] p-5"
                        >
                          <div
                            className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${STATUS_LABELS[status].tone}`}
                          >
                            {String(counts[status]).padStart(2, "0")}
                          </div>
                          <h2 className="mt-4 text-xl font-semibold text-[#1a2332]">
                  ...
                  <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-start justify-between gap-4">
                                  ...
                                  <span
                                    className={`rounded-full px-3 py-1 text-xs font-semibold ${STATUS_BADGE[status]}`}
                                  >
                                    {status === "full"
                                      ? "Online"
                                      : status === "partial"
                                ...
                                <p className="mt-4 text-sm text-slate-500">
                                  {station.status
                                    ? `Last update: ${new Date(station.status.last_updated).toLocaleString()}`
                                    : "No data yet"}
                                </p>
                              </article>
                            );
                    ...
                    <aside className="rounded-[28px] border border-slate-200 bg-[#f8fafc] p-5">
                      ...
                      <p className="mt-4 text-sm leading-7 text-slate-600">
                        AI-driven diagnostics and station insights will appear
                        here once the monitoring model is connected.
                      </p>
                    </aside>
                  </section>
                </>
