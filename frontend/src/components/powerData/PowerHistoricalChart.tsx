      ...
      <div className="relative" style={{ maxWidth: '100%' }}>
        ...
        >
          ...
          {yTicks.map((v) => {
            ...
            return (
              <g key={v}>
                <line x1={PAD.left} y1={y} x2={SVG_W - PAD.right} y2={y} stroke="var(--surface-tertiary)" strokeWidth="0.5" />
          ...
          {primaryPath && (
            <>
              {(() => {
                ...
                const firstPt = primaryPath.match(/M\s+([\d.]+),/)?.[1]
                const lastPt = [...primaryPath.matchAll(/([\d.]+),[\d.]+(?:\s|$)/g)].pop()?.[1]
                const bottomY = PAD.top + cH
                const areaPath = firstPt && lastPt
                  ? `${primaryPath} L ${lastPt},${bottomY} L ${firstPt},${bottomY} Z`
                  : ''
          ...
          {tooltip && (
            <>
              ...
              {tooltip.primaryValue !== null && (
                <circle cx={tooltip.x} cy={syVal(tooltip.primaryValue)} r="4" fill={cfg.color} stroke="var(--color-surface)" strokeWidth="2" />
              )}
            </>
          )}
        </svg>
